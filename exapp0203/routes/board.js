...
router.put('/', function(request, response, next) {
  ...
  con.query(sql,[title, writer, content, board_id] ,function(error, result, fields){
    if(error){
      ...
      message.data=result;
      message.msg="수정하기 성공";
      response.end(JSON.stringify(message));//웹 요청에 대한 응답

      broadCasting(message);//웹 소켓을 이용한 브로드캐스팅
    }
    con.end();//접속끊기
  });
...
router.delete('/:board_id', function(request, response, next) {
  ...
  con.query(sql,[board_id] ,function(error, result, fields){
    if(error){
      ...
      response.end(JSON.stringify(message));//웹 요청 응답
      broadCasting(message);//웹 소켓 브로드캐스팅
    }
    con.end();//접속끊기
  });
});
