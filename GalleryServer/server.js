...
app.use(static(__dirname+"/static/images"));//사용하고픈 미들웨어명
...
app.post("/gallery",function(request,response){
    ...
    con.query(sql,[],function(error,results,fields){// 바인드 변수값..
        if(error){
...
app.get("/gallery",function(request,response){
    var con = mysql.createConnection(conStr);
    con.query("select * from gallery",function(error,result,fields){
        if(error){
            response.writeHead(500,{"Content-Type":"application/json;charset=utf-8"});
            response.end("{}");
        }else{
            ...
            response.end(JSON.stringify(results));  // 레코드 결과 자체가 json배열이므로, 스트링화 해서 응답
        }
    });
});
